...
- Stop the recurring benign Doctor `stale-lock` noise for finished issue workers. The issue-dispatch lock (`state/dispatch_<n>.lock`) was created in `Dispatcher#dispatch` but — unlike the ci-fix/plan/pr/doctor locks — never recorded the worker pid. When a worker finished cleanly but `StatusTransitions` didn't clear the lock (the card had already left *In progress* by the time a transition would run — PR merged, card in review/done), the lock had no owner and nothing reaped it, so it aged the full 1h TTL. Because the watchdog's `stale_lock_minutes` threshold (45m) fires first, a Doctor was spawned to investigate an already-finished, harmless lock — repeatedly (observed on `njcameron/rh-app` locks 678/680/682). `Dispatcher#dispatch` now calls `record_pid` after spawning, and `DispatchLock#reap_finished_issue_locks` (run each orchestrator tick) releases any issue lock whose recorded worker pid has exited — within a tick instead of ageing into the alarm. Only locks with a recorded pid are reaped, so a legacy ownerless lock is never freed out from under a live worker. Regression tests cover the pid recording and the reap sweep (dead/live/ownerless/prefixed-lock cases). (#17)
